...
When β_within = 0, a mediation analysis through MC would still produce a significant indirect effect — because the pooled path from condition → MC and from MC → DV are both non-zero. But that indirect effect is entirely a *group-level compositional artifact*, not evidence that MC mediates any psychological process. The groups differ on DV for reasons that have nothing to do with where individual participants fall on the MC within their group.

This is a version of the ecological fallacy: group-level patterns do not necessarily reflect individual-level relationships. Before reporting mediation, always check whether the construct-to-DV relationship holds *within* conditions, not just in the pooled sample.
:::
...
# A Quiet Bias: Asymmetric Measurement Error on Bounded Scales {#sec-bounded-error}
...
// Truncated normal PDF grid for density plot
{
  const lo = Math.max(loB === -Infinity ? bound_mu - 4*bound_sd : loB, bound_mu - 5*bound_sd);
  const hi = Math.min(hiB ===  Infinity ? bound_mu + 4*bound_sd : hiB, bound_mu + 5*bound_sd);
}
...
```{ojs}
//| echo: false
//| fig-cap: "Density of observed scores on the bounded scale. Dashed black = true mean; blue = observed mean; red = observed median. The gap between true and observed mean shows the truncation bias. Drag the sliders to see how proximity to a bound and variability increase the bias."

Plot.plot({
  width: 650, height: 300,
  marginLeft: 55, marginBottom: 45, marginTop: 20, marginRight: 20,
  x: {label: "Scale value", domain: [Math.min(loB === -Infinity ? bound_mu - 4.5*bound_sd : loB - 2, -5),
                                       Math.max(hiB === Infinity ? bound_mu + 4.5*bound_sd : hiB + 2, 105)]},
  y: {label: "Density", domain: [0, null]},
  marks: [
    Plot.areaY(density_data, {x: "x", y: "pdf", fill: "#457b9d", fillOpacity: 0.25, curve: "natural"}),
    Plot.lineY(density_data, {x: "x", y: "pdf", stroke: "#457b9d", strokeWidth: 2, curve: "natural"}),
    // True mean
    Plot.ruleX([bound_mu], {stroke: "#222", strokeDasharray: "6,3", strokeWidth: 2}),
    // Observed mean
    Plot.ruleX([trunc_moments.obs_mean], {stroke: "#457b9d", strokeWidth: 2.5}),
    // Observed median
    Plot.ruleX([trunc_moments.obs_med],  {stroke: "#e63946", strokeWidth: 2.5, strokeDasharray: "4,3"}),
    // Boundary markers
    ...(loB > -Infinity ? [Plot.ruleX([loB], {stroke: "#888", strokeWidth: 1.5})] : []),
    ...(hiB <  Infinity ? [Plot.ruleX([hiB], {stroke: "#888", strokeWidth: 1.5})] : []),
    // Legend text
    Plot.text([{x: bound_mu, label: "True mean", y: 0}],
      {x: "x", y: d => Math.max(...density_data.map(p=>p.pdf)) * 1.05, text: "label",
       fill: "#222", fontSize: 10, textAnchor: "start", dx: 4}),
  ]
})
```
