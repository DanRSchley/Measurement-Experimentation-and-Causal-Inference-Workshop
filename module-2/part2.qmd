...
<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vTampW1IhJLthWX0znIsCs4Dsz_AGj8VDEErcPXdRQ02wugmdGKE4RBi86qp-PSQQ/pubembed?start=false&loop=false&delayms=3000" width="100%" height="500px" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" title="Module 2 Part 2: Exclusion Restriction"></iframe>
...
The researcher intends the eco-label to be the only thing that differs between conditions. But in preparing the stimuli, the two versions of the coffee bag were not held perfectly equal: the eco-label condition also happened to use a **more premium-looking package design** — darker color scheme, cleaner typography, higher perceived quality cues. Participants in the eco-label condition therefore receive a *bundle*: eco-certification **plus** a premium visual presentation. Both signals independently increase WTP through different pathways.
...
```{r part2-setup}
#| code-summary: "▶ Simulate 120 participants with confounded eco-label and premium design"
...
```{r confound-decomposition-plot, fig.height=5}
#| code-summary: "▶ Plot: decompose observed effect into eco-certification vs. premium design paths"
...
The plot shows what a researcher cannot see from the t-test alone. **Path A** — the true eco-certification effect — varies by participant: participants with many eco-responsive traits gain more from the label, while those with few traits gain little. **Path B** — the premium design effect — is a flat $`r avg_B` lift for everyone in the eco-label condition, regardless of any trait.

When the researcher reports the overall WTP difference of $`r naive`, they attribute the entire effect to eco-certification. But roughly $`r avg_B` of that is driven by the premium package design — a confound that was never randomized away because it was never measured. The true average eco-label effect across participants is only $`r avg_A`, not $`r naive`.

The exclusion restriction failed: **the manipulation injected two latent signals into a single condition dummy**, and the treatment coefficient absorbs both.
...
## Researcher Checklist: The Exclusion Restriction {#sec-checklist-part2}

::: {.callout-note}
## Designing the manipulation

- [ ] **Does your manipulation vary only the theoretically intended construct?** Write down every feature of the treatment condition that differs from the control condition. Each difference is a potential violation of the exclusion restriction.
- [ ] **Is each additional feature *theoretically independent* of the construct you are studying?** A manipulation that changes eco-label, packaging quality, and premium price signals simultaneously is not testing the eco-label — it is testing a bundle.
- [ ] **Can you create a "process" control condition** that holds all features constant except the theoretically relevant one? If not, your treatment is confounded by design.
- [ ] **Did you pilot-test the manipulation** using open-ended responses (asking what participants noticed or thought about)? Pilot text will reveal which features of the treatment are psychologically active beyond the intended mechanism.
:::

::: {.callout-note}
## Diagnosing exclusion restriction violations

- [ ] **Does the manipulation check for the intended mechanism stay significant when you control for the alternative mechanisms you measured?** If controlling for perceived quality eliminates the eco-label manipulation check effect, the manipulation was working mostly through quality signals, not sustainability beliefs.
- [ ] **Have you measured all plausible mediating mechanisms activated by your treatment?** Unmeasured active mechanisms are exclusion restriction violations you cannot detect. Use a nomological net scan (open-ended pilot text, existing literature) to enumerate all pathways before data collection.
- [ ] **If the treatment effect disappears after controlling for an alternative mediator**, is this because that variable is the true causal mechanism, or because your treatment is confounded with it? This distinction requires clear theoretical reasoning — and potentially a factorial design to disentangle them.
:::

::: {.callout-note}
## Connection to Module 1 discriminant validity and Module 3 mediation

- [ ] **Is the exclusion restriction for your treatment the experimental analog of discriminant validity for your scale?** In both cases, the observable (treatment / scale score) picks up variance from multiple latent constructs simultaneously. Apply the same discipline: map the latent constructs affected before running the study.
- [ ] **If your treatment has multiple active pathways, is decomposing them theoretically important?** A manipulation that works through eco-beliefs, quality perception, and price anchoring simultaneously tells you little about which mechanism to strengthen or suppress. Module 3, Part 5 (mediation analysis) shows how to decompose the total effect into component pathways.
- [ ] **Have you considered a factorial design to isolate the mechanism?** If Pathway A and Pathway B are both active, a 2×2 design varying each independently can test which carries the effect — without needing a single-mechanism manipulation.
:::

*The central question for this part: Does my manipulation do one thing theoretically — or does it do several things at once, making my coefficient an uninterpretable mixture of multiple causal pathways?*
