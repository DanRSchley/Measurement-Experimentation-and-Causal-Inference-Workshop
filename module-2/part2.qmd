...
When the researcher reports the overall WTP difference of $`r naive`, they attribute the entire effect to eco-certification. But roughly $`r avg_B` of that is driven by the premium package design — a signal that was perfectly confounded with the eco-label because it was never measured or separated. The eco-certification-specific effect across participants is only $`r avg_A`, not $`r naive`.
