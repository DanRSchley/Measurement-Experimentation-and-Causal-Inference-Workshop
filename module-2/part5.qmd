---
title: "Part 5: Experimental Design Strategies and Assignment Solutions"
---
...
<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vQnz6Vpt7vp1ADOrFLktNe2nhQI99W5TbY1lJ7FH6jk9VA5bY618C_nx4-FzQsJdA/pubembed?start=false&loop=false&delayms=3000" width="100%" height="500px" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" title="Module 2 Part 5: Experimental Design"></iframe>

# Part 5: Experimental Design Strategies — Using Structure to Buy Efficiency {#sec-designs}

## The Core Tension

Parts 2, 3, and 4 showed a fundamental tension:
...
viewof ls_config = Inputs.form({
  ...
  a_levels:   Inputs.select([2, 3, 4], {label: "Factor A — levels", value: 3}),
  ...
  c_levels:   Inputs.select([2, 3, 4], {label: "Factor C — levels", value: 3}),
...
The tradeoff is real: **carryover effects** (condition A changes how the person responds to condition B), **demand characteristics** (participants figure out the hypothesis), **order effects** (earlier conditions shift baseline responding), and **fatigue or practice effects** all threaten the validity of within-person contrasts. These threats are most severe when:

- The treatment induces lasting attitude change (e.g., persuasion studies, value activation)
- The conditions are highly transparent and the hypothesis is easy to infer
- The outcome requires skill or memory that improves with practice
- Conditions cannot be effectively counterbalanced (e.g., one condition is a natural "first" experience)

For short perceptual or preference tasks — including most WTP and product evaluation paradigms — within-participant designs are often feasible and substantially more efficient, provided conditions are counterbalanced and order effects are checked. Choose deliberately based on the construct; do not adopt within-participant designs as a default efficiency gain without verifying that the tradeoffs are acceptable for your specific question.
...
- **$\sigma^2/N$ is fixed.** It is set entirely by your total budget. Adding more microstudies does not change it.
- **$\tau^2/K$ shrinks** as K grows. Each new microstudy samples another region of the scenario space, averaging out more of the between-scenario heterogeneity.
...
#| code-summary: "▶ Plot: metastudy power rises with K; separate A/B tests collapse"
...
::: {.callout-note}
## Structural designs: what each buys and what each costs

Structural designs buy efficiency by moving variance from the error term into the design itself. But each changes the estimand and introduces its own assumptions:

- **Within-person designs** remove stable between-person confounding for within-person contrasts — but introduce carryover, order, fatigue, practice, and demand risks. The efficiency gain is largest when between-person ICC is high and conditions are easy to counterbalance; it may be erased or reversed when conditions are transparent or treatments have lasting effects.
- **Latin squares** achieve approximate balance on main effects with fewer cells than a full factorial — but standard unreplicated Latin squares cannot estimate interactions, and if interactions are present, the main-effect estimates are uninterpretable. Use a Latin Square only when interaction effects are theoretically implausible or unimportant.
- **Metastudies** improve power for the **average effect across sampled scenarios**, not for any individual microstudy. The individual microstudy is necessarily underpowered. But testing the average effect is typically the right estimand when a hypothesis is supposed to hold generally — which is precisely what most social-science hypotheses claim.

No single design dominates all others. Choose based on which threats are most dangerous for your specific construct and question.
:::
...
## Researcher Checklist: Experimental Design {#sec-checklist-part5}

::: {.callout-note}
## Key questions about your design structure

- [ ] **Does your design match your theoretical question?** A main-effect-only hypothesis needs a simple A/B or factorial. An interaction hypothesis requires at least a 2×2 — a single A/B cannot test whether an effect is moderated.
- [ ] **How many stimuli are you using?** A single advertisement, brand, or scenario is one draw from the stimulus space. Your conclusion can only generalize to that specific stimulus unless you use multiple exemplars and treat stimulus as a random effect.
- [ ] **Is the efficiency gain of a within-subjects design worth the carryover risk for your construct?** Within-subjects removes between-person variance (more power) but introduces demand characteristics and order effects. Choose deliberately, not by default.
- [ ] **Have you pre-registered the design?** Post-hoc flexibility in which cells to compare, which interactions to test, or which conditions to collapse inflates false-positive rates — this is the design-level analog of researcher degrees of freedom.
:::
