...
The parallel mediation and SEM above address *which* mediators to include and *how* to model them. Two additional threats remain regardless of model specification: **non-normal residuals / influential outliers** (which can corrupt OLS *b*-path estimates) and **unmeasured M–Y confounders** (which bias the ACME even when X is randomly assigned). Each problem has a targeted remedy.

---

#### Threat 1: Outlier Contamination → Robust MM-Estimation (ROBMED)

Standard OLS mediation uses the product of two OLS coefficients (*a* × *b*). OLS is sensitive to influential outliers: even a small fraction of extreme values in M or Y can substantially distort one or both regression coefficients, and the product amplifies the distortion. The solution is to replace OLS with a **high-breakdown MM-estimator** — the core statistical idea behind ROBMED (Alfons, Ates, & Groenen, 2021). The MM-estimator has an ≈50% breakdown point, meaning the estimates remain bounded even if up to half the sample consists of outliers.

```{r mediation-outlier, fig.height=4.5, fig.width=9}
#| code-summary: "▶ Robust MM-estimation vs. OLS bootstrap under outlier contamination"

set.seed(2025)
library(MASS)   # rlm() MM-estimator (base recommended package)

N_m <- 300
X_m <- rep(c(0L, 1L), each = N_m / 2)   # balanced; true ACME = a × b = 0.50 × 0.40 = 0.20

# ── Clean DGP ─────────────────────────────────────────────────────────────────
M_cl <- 0.50 * X_m + rnorm(N_m)                 # a-path = 0.50
Y_cl <- 0.40 * M_cl + 0.10 * X_m + rnorm(N_m)  # b-path = 0.40; true ACME = 0.20

# ── Outlier-contaminated DGP ──────────────────────────────────────────────────
# Replace 10% of M and 5% of Y with draws that are 6× the normal SD.
# True structural parameters (a, b) are UNCHANGED — the contamination is in the
# noise, not the signal. True ACME is still 0.20.
contam <- function(x, frac, scale = 6) {
  x[sample(length(x), floor(length(x) * frac))] <-
    rnorm(floor(length(x) * frac), 0, scale)
  x
}
M_ou <- contam(M_cl, frac = 0.10)
Y_ou <- contam(Y_cl, frac = 0.05)

df_cl <- data.frame(X = X_m, M = M_cl, Y = Y_cl)
df_ou <- data.frame(X = X_m, M = M_ou, Y = Y_ou)

# ── Helper: OLS bootstrap mediation (product-of-coefficients + percentile CI) ─
ols_boot_med <- function(d, R = 200) {
  ab  <- unname(coef(lm(M ~ X, d))["X"] * coef(lm(Y ~ M + X, d))["M"])
  n   <- nrow(d)
  bts <- replicate(R, {
    s <- d[sample(n, n, replace = TRUE), ]
    unname(coef(lm(M ~ X, s))["X"] * coef(lm(Y ~ M + X, s))["M"])
  })
  c(est = ab, lo = quantile(bts, 0.025), hi = quantile(bts, 0.975))
}

# ── Helper: robust MM-estimation bootstrap ────────────────────────────────────
# rlm(method = "MM") simultaneously estimates an MM-scale and MM-regression
# coefficients.  The breakdown point is ≈50%: estimates remain unaffected even
# if up to half the observations are outliers.  This is the estimator underlying
# ROBMED (Alfons, Ates & Groenen, 2021).
rob_mm_boot <- function(d, R = 200) {
  ab <- tryCatch(
    unname(coef(rlm(M ~ X, d, method = "MM"))["X"] *
           coef(rlm(Y ~ M + X, d, method = "MM"))["M"]),
  ...
  bts <- replicate(R, {
    s <- d[sample(n, n, replace = TRUE), ]
    tryCatch(
      unname(coef(rlm(M ~ X, s, method = "MM"))["X"] *
             coef(rlm(Y ~ M + X, s, method = "MM"))["M"]),
      error = function(e) NA_real_
    )
  })
  bts <- bts[!is.na(bts)]
...
ols_cl <- ols_boot_med(df_cl);   ols_ou <- ols_boot_med(df_ou)
rob_cl <- rob_mm_boot(df_cl);    rob_ou <- rob_mm_boot(df_ou)

cat(sprintf("Clean DGP   OLS: %.3f [%.3f, %.3f]   Robust MM: %.3f [%.3f, %.3f]\n",
            ols_cl["est"], ols_cl["lo"], ols_cl["hi"],
            rob_cl["est"], rob_cl["lo"], rob_cl["hi"]))
cat(sprintf("Outlier DGP OLS: %.3f [%.3f, %.3f]   Robust MM: %.3f [%.3f, %.3f]\n",
            ols_ou["est"], ols_ou["lo"], ols_ou["hi"],
            rob_ou["est"], rob_ou["lo"], rob_ou["hi"]))

# ── Recovery plot ──────────────────────────────────────────────────────────────
rob_df <- rbind(
  data.frame(Method = "OLS bootstrap",
             DGP = "Clean (no outliers)",
             est = ols_cl["est"], lo = ols_cl["lo"], hi = ols_cl["hi"]),
  data.frame(Method = "Robust MM-estimation",
             DGP = "Clean (no outliers)",
             est = rob_cl["est"], lo = rob_cl["lo"], hi = rob_cl["hi"]),
  data.frame(Method = "OLS bootstrap",
             DGP = "10% outliers in M / 5% in Y",
             est = ols_ou["est"], lo = ols_ou["lo"], hi = ols_ou["hi"]),
  data.frame(Method = "Robust MM-estimation",
             DGP = "10% outliers in M / 5% in Y",
             est = rob_ou["est"], lo = rob_ou["lo"], hi = rob_ou["hi"])
) |>
  mutate(
    Method = factor(Method, levels = c("OLS bootstrap", "Robust MM-estimation")),
    DGP    = factor(DGP, levels = c("Clean (no outliers)", "10% outliers in M / 5% in Y"))
  )

ggplot(rob_df, aes(x = est, y = Method, color = Method)) +
  geom_vline(xintercept = 0.20, linetype = "dashed", color = "gray40", linewidth = 0.8) +
  geom_errorbarh(aes(xmin = lo, xmax = hi), height = 0.25, linewidth = 0.9) +
  geom_point(size = 3.5) +
  scale_color_manual(values = c("OLS bootstrap"       = "#4a90d9",
                                 "Robust MM-estimation" = "#d7301f"),
                     guide = "none") +
  facet_wrap(~ DGP, ncol = 2) +
  labs(x = "Estimated ACME (indirect effect)", y = NULL,
       title = "Robust MM-Estimation vs. OLS Bootstrap: Outlier Sensitivity",
       subtitle = "Dashed line = true ACME (0.20)  \u00b7  Error bars = 95% bootstrap CI") +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank(),
...
::: {.callout-note}
## What the ROBMED plot shows

**On clean data, both methods recover the truth (0.20).** The MM-estimator behaves like OLS when residuals are normal — there is no cost to using it on well-behaved data.

**On contaminated data, OLS drifts away from 0.20** because even a small fraction of extreme M or Y values inflates the residual variance and pulls the OLS regression line toward the outliers. Each outlier acts as a high-leverage point that distorts the *b*-path, and the product *a* × *b* amplifies the distortion.

**The robust MM-estimator stays close to 0.20.** It assigns near-zero weight to observations with large robust residuals, effectively ignoring the outliers when computing the regression coefficients. The breakdown point of ≈50% means the estimates remain stable as long as fewer than half the sample are outliers.

**When to use it:** whenever measurement error, data entry mistakes, or heavy-tailed constructs could produce influential extreme values in M or Y. ROBMED has a small efficiency cost on clean data (slightly wider CIs) but substantial protection on contaminated data — a favorable trade-off in practice.
:::

---

#### Threat 2: Unmeasured M–Y Confounding → Imai's Sensitivity Analysis

Even with random assignment of X, a hidden variable U that causes **both M and Y** biases the ACME estimate. X → M remains clean (X is randomized), but the estimated *b*-path (M → Y) is confounded: M and Y co-vary partly through U, not just through the structural M → Y path. The result is a biased ACME — typically **inflated** when U has the same-sign effects on M and Y.

No estimator can remove this bias without measuring U. The remedy is instead to *characterize* the bias via **sensitivity analysis** (Imai, Keele, & Tingley, 2010). The key parameter is **ρ**: the correlation between the M equation's error term and the Y equation's error term. ρ = 0 means the no-unmeasured-confounder assumption holds. ρ ≠ 0 means a hidden U exists that correlates the two error terms. Sensitivity analysis asks: *how large would ρ need to be to reduce the OLS ACME to zero?* Call this threshold **ρ\***. If ρ\* is large, the result is robust; if ρ\* is small, even modest confounding would overturn the conclusion.

```{r mediation-confounded, fig.height=4.5, fig.width=9}
#| code-summary: "▶ Imai sensitivity analysis: unmeasured M\u2013Y confounder"
...
# ── Confounded DGP ─────────────────────────────────────────────────────────────
# U is an unmeasured common cause of M and Y (U is NOT caused by X).
# Structural paths: a = 0.50 (X→M), b = 0.40 (M→Y)  →  true ACME = 0.20.
#
# U loadings: γ = 0.70 on M residuals, δ = 0.70 on Y residuals.
# When Var(M residual) = Var(Y residual) = 1, the M–Y residual correlation is:
#   ρ_DGP = γ × δ = 0.70 × 0.70 = 0.49
#
# Without observing U, OLS inflates the b-path by ≈ γ × δ = 0.49:
#   b̂_OLS ≈ 0.40 + 0.49 = 0.89  →  ACME_OLS ≈ 0.50 × 0.89 ≈ 0.45  (vs. truth 0.20)

gamma_u <- 0.70;   delta_u <- 0.70
rho_dgp <- gamma_u * delta_u   # = 0.49

U_c    <- rnorm(N_m)
M_conf <- 0.50 * X_m + gamma_u * U_c + rnorm(N_m, 0, sqrt(1 - gamma_u^2))
Y_conf <- 0.40 * M_conf + 0.10 * X_m + delta_u * U_c + rnorm(N_m, 0, sqrt(1 - delta_u^2))
df_conf <- data.frame(X = X_m, M = M_conf, Y = Y_conf)

# ── Fit Imai's causal mediation model ─────────────────────────────────────────
fit_m_imai <- lm(M ~ X, data = df_conf)
fit_y_imai <- lm(Y ~ M + X, data = df_conf)
med_imai   <- mediate(fit_m_imai, fit_y_imai,
                      treat = "X", mediator = "M",
                      boot = TRUE, sims = 200)

# ── Sensitivity analysis: vary ρ in [−1, 1] and track ACME(ρ) ────────────────
# medsens() asks: "if the M and Y errors had correlation ρ (due to some hidden U),
# what ACME would we estimate?"  At ρ = 0 it returns the naïve OLS estimate;
# as |ρ| increases toward the true DGP level, the corrected ACME approaches 0.
sens_imai <- medsens(med_imai, rho.by = 0.025, effect.type = "indirect")

rho_seq  <- sens_imai$rho
acme_seq <- sens_imai$d0

# ρ* = smallest positive ρ that nullifies the ACME (interpolated crossing point)
pos_idx  <- rho_seq >= 0 & !is.na(acme_seq)
rho_crit <- tryCatch(
  approx(acme_seq[pos_idx], rho_seq[pos_idx], xout = 0)$y,
  error = function(e) NA_real_
)

cat(sprintf("True ACME                 : 0.200\n"))
cat(sprintf("OLS (naïve) ACME          : %.3f  [%.3f, %.3f]\n",
            med_imai$d0, med_imai$d0.ci[1], med_imai$d0.ci[2]))
cat(sprintf("\u03c1* (ACME \u2192 0 threshold)  : %.3f\n", rho_crit))
cat(sprintf("\u03c1_DGP (true confounding)  : %.3f\n", rho_dgp))
cat(sprintf("Verdict: \u03c1_DGP (%.2f) > \u03c1* (%.2f) \u2192 true confound is large enough to nullify OLS result\n",
            rho_dgp, rho_crit))

# ── Sensitivity plot ──────────────────────────────────────────────────────────
sens_df  <- data.frame(rho = rho_seq, acme = acme_seq) |> filter(!is.na(acme))

ggplot(sens_df, aes(x = rho, y = acme)) +
  geom_hline(yintercept = 0,    color = "gray50", linewidth = 0.7) +
  geom_hline(yintercept = 0.20, color = "#d7301f", linetype = "dotted", linewidth = 1) +
  geom_line(color = "#4a90d9", linewidth = 1.2) +
  # ρ* threshold
  geom_vline(xintercept = rho_crit,
             linetype = "dashed", color = "gray40", linewidth = 0.8, na.rm = TRUE) +
  # True DGP confounding level
  geom_vline(xintercept = rho_dgp,
             color = "#d7301f", linetype = "dashed", linewidth = 1) +
  annotate("text",
           x = if (!is.na(rho_crit)) rho_crit + 0.015 else 0.25,
           y = 0.04,
           label = if (!is.na(rho_crit))
             paste0("\u03c1* = ", round(rho_crit, 2), "\n(ACME \u2192 0)")
           else "",
           hjust = 0, size = 3.5) +
  annotate("text",
           x = rho_dgp + 0.015, y = max(sens_df$acme) * 0.72,
           label = paste0("True DGP \u03c1 = ", round(rho_dgp, 2)),
           hjust = 0, color = "#d7301f", size = 3.5) +
  annotate("text",
           x = min(sens_df$rho), y = 0.225,
           label = "True ACME = 0.20",
           hjust = 0, color = "#d7301f", size = 3.5) +
  labs(
    x = expression(paste(rho, "  (correlation between M and Y error terms)")),
    y = "Estimated ACME",
    title = "Imai Sensitivity Analysis: How Fragile Is the ACME Estimate?",
    subtitle = paste0(
      "Blue line = ACME(\u03c1)  \u00b7  grey dashed = \u03c1* (ACME \u2192 0)  \u00b7  ",
      "red dashed = true DGP confounding  \u00b7  red dotted = true ACME"
    )
  ) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank())
...
::: {.callout-note}
## Reading the sensitivity analysis plot

**The x-axis is ρ**, the hypothetical correlation between the M equation's error term and the Y equation's error term. This correlation arises when an unmeasured variable U causes both M and Y. If U has loading γ on M (residual) and δ on Y (residual), and both residual variances equal 1, then ρ ≈ γ × δ. In this DGP, γ = δ = 0.70, so ρ_DGP = 0.49.

**The blue curve** is ACME(ρ): the ACME estimate you would obtain *if* you corrected for confounding at level ρ. At ρ = 0 (left side, no confounding assumed), the curve returns the naïve OLS estimate — inflated by the unmodeled U. As ρ increases toward the true DGP level (red dashed line), the corrected ACME approaches the true value (0.20, red dotted line) and then continues to 0.

**ρ\* (grey dashed)** is the critical threshold: the smallest ρ that would make the ACME indistinguishable from zero.

- If **ρ\* is large** (e.g., > 0.50): even strong confounding would leave a non-zero ACME → result is **robust**.
- If **ρ\* is small** (e.g., < 0.20): even modest confounding would nullify the result → result is **fragile**.

**In this simulation**, ρ_DGP (0.49) exceeds ρ\*, confirming that the true level of confounding *is* sufficient to bring the OLS ACME to zero — which is exactly what we observe (true ACME = 0.20, not ≈ 0.45). **Imai's approach does not fix the bias** — it *characterizes* how large ρ would need to be to overturn the conclusion, giving readers a principled basis for assessing the plausibility of unmeasured confounding.

**Practical guidance:** after running the sensitivity analysis, ask whether a confound large enough to produce ρ = ρ\* is scientifically plausible in your setting. If yes, you cannot confidently claim mediation without measuring (or instrumenting) U. The nomological net scanning approach from Step 1 is the pre-emptive solution: identify potential M–Y confounders *before* data collection, measure them as covariates, and include them in both the M and Y equations.
:::
...
|---|---|
| Scale items | Treatment conditions |
...
danger_df <- err_df |>
  ...
  select(N, t1_rate, t2_rate, pct_fail) |>
