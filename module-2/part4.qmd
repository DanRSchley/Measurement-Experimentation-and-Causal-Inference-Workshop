...
meta_se_fn <- function(K) sqrt(tau^2 / K + sigma^2 / N_total)
...
**Reading the plot:** at K = 1, the separate A/B test has higher power (~98%) because it runs one large, clean study with all 400 participants. The metastudy starts lower (~35%) because with only one microstudy you have no way to know whether your single chosen scenario is representative — the effect heterogeneity τ adds substantial uncertainty to the meta-analytic estimate. As K grows, those roles reverse. Each new microstudy samples a different region of the scenario space, and the τ²/K term in the SE formula shrinks rapidly. By K = 3, the metastudy already surpasses the A/B test's power; by K = 15, it reaches near-ceiling power (>99%) while the separate A/B tests have collapsed below 20% (each test now has only ~27 participants). The shaded region shows precisely how much of the metastudy's power gain comes from reducing the heterogeneity component — and it is substantial.
