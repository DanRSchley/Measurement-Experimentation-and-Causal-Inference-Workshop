...
# ── Parameters (inspired by DeKay et al., 2022, Experiment 2) ────────────────
# d   = average true effect size (Cohen's d) across all scenarios
# tau = SD of true effects across scenarios (heterogeneity); tau/d ≈ 0.63
#       mirrors the paper's reported ratio of ~0.71 for the framing effect
...
#
# Metastudy: K microstudies, each n1 = N/K participants (between-person design)
#   SE_meta = sqrt(tau^2 / K  +  sigma^2 / N)
#   Note: sigma^2 / (n1 * K) = sigma^2 / ((N/K) * K) = sigma^2 / N — fixed!
#   Only the tau^2/K term shrinks as K grows → SE shrinks → power rises.
#
...
# At K=1 a metastudy IS a single A/B test — the tau^2/K scenario-sampling term
# does not apply because there is only one scenario and no averaging across scenarios.
# For K >= 2 the DeKay et al. formula applies: SE = sqrt(tau^2/K + sigma^2/N).
meta_se_fn <- function(K) {
  ifelse(K == 1,
         sigma * sqrt(4 / N_total),          # = ab_se_fn(1): same as one A/B test
         sqrt(tau^2 / K + sigma^2 / N_total))
}
...
# ── SE-component annotation data ─────────────────────────────────────────────
se_df <- data.frame(K = K_vec) |>
  mutate(
    se_total    = meta_se_fn(K),
    se_floor    = sqrt(sigma^2 / N_total),   # lower bound: tau^2/K → 0
    se_tau_only = sqrt(tau^2 / K)            # heterogeneity component alone
  )
...
ggplot(power_df, aes(x = K, y = power, color = design, group = design)) +
  ...
  # shaded region showing heterogeneity reduction benefit
  geom_ribbon(
    data = se_df,
    aes(x = K,
        ymin = power_fn(d, se_total),
        ymax = power_fn(d, se_floor)),
    inherit.aes = FALSE,
    fill = "#2d6a4f", alpha = 0.10
  ) +
  geom_line(linewidth = 1.3) +
  geom_point(size = 3.5) +
  annotate("text", x = 16, y = 0.90,
           label = "Shaded area = power gain\nfrom reducing \u03c4\u00b2/K",
           color = "#2d6a4f", size = 3.0, hjust = 0, fontface = "italic") +
  ...
  labs(
    ...
    title    = "Metastudy Power RISES with K; Separate A/B Tests Collapse",
    subtitle = paste0(
      "Both designs use N = ", N_total, " total  \u00b7  ",
      "d = ", d, "  \u00b7  \u03c4 = ", tau,
      " (SD of true effect across scenarios)\n",
      "Metastudy SE = \u221a(\u03c4\u00b2/K + \u03c3\u00b2/N): ",
      "the \u03c4\u00b2/K term shrinks as K grows, so SE falls and power rises  \u00b7  ",
      "A/B SE \u221d 1/\u221a(N/K): shrinking budget per test collapses power"
    )
  ) +
...
**Reading the plot:** at K = 1, both designs are identical — a metastudy with a single microstudy is just an A/B test, and they share the same SE and therefore the same power (~98% for d = 0.40 with N = 400). As K grows, the two designs diverge sharply in opposite directions. In the metastudy, each new microstudy samples a different region of the scenario space, and the τ²/K term in the SE formula shrinks rapidly — so power rises. In the separate A/B tests, each individual study receives only N/K participants, so the per-test SE grows and power falls. By K = 3, the metastudy already surpasses the A/B test's power; by K = 15, it reaches near-ceiling power (>99%) while the separate A/B tests have collapsed below 20% (each test now has only ~27 participants). The shaded region shows precisely how much of the metastudy's power gain comes from reducing the heterogeneity component τ²/K — and it is substantial.
