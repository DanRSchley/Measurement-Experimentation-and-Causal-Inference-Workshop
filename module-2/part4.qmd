...
The first is that in real experiments, exclusion criteria almost never screen participants who are *randomly* low-quality: a participant who speeds through the survey is also likely to give less considered responses — a lower WTP estimate, a weaker attitude rating, a noisier preference judgment. Exclusion criteria (RT, attention checks, quality scores) are **correlated with the outcome Y**. When you remove low-RT or low-attention participants, you are simultaneously removing the low-Y tail of the distribution. That asymmetric truncation makes the standard error look smaller than it is, so the t-test is anti-conservative — it rejects H₀ more than 5% of the time even when the null is true.

The second problem is researcher degrees of freedom: when the exclusion rule is chosen *after seeing the data* — especially after seeing whether the treatment effect is significant — the choice of rule is itself a researcher degree of freedom. Across all rules a researcher might try, the probability that at least one crosses α = .05 is far higher than 5%.

The simulation below captures both problems. The DGP has a latent **engagement** variable that drives response time, outcome quality, and Y. Critically, the treatment condition (eco-label) is more novel and engaging than the control, so treatment **boosts engagement by 0.5 SD**. H₀ is still true on Y (Y depends only on baseline engagement, not on the treatment-induced boost). But RT and quality reflect the boosted engagement — so applying pooled exclusion thresholds removes more participants from the less-engaging control condition than from the treatment condition. This asymmetric exclusion, combined with the engagement→Y path, inflates individual-rule FP rates. Choosing the most favorable rule inflates them further.

```{r exclusion-bias, fig.height=5.5, fig.width=11}
#| code-summary: "▶ Simulate: false-positive rate across 10 exclusion rules (null effect)"
...
sim_pvals <- replicate(n_sims_ex, {
  ...
  # Baseline engagement trait: independent of treatment (individual difference)
  engagement <- rnorm(N_excl)

  # Treatment (eco-label) is more engaging than control.
  # H₀ is still true: Y has zero treatment effect. But engagement predicts Y strongly,
  # and treatment boosts effective engagement → exclusion criteria are correlated with
  # BOTH the DV (Y) and condition — exactly the André (2022) hazard.
  engagement_boost <- 1.0
  effective_eng    <- engagement + engagement_boost * trt

  # Y (WTP): strongly driven by baseline engagement, NOT by treatment directly.
  # corr(Y, engagement) ≈ 0.85 so exclusion on engagement-correlated criteria bites hard.
  Y  <- 0.85 * engagement + sqrt(1 - 0.85^2) * rnorm(N_excl)

  # RT and quality depend on EFFECTIVE engagement (including the boost).
  # Tight noise → pooled threshold cuts asymmetrically into control distribution.
  rt      <- 5 + 3 * effective_eng + rnorm(N_excl, sd = 1.5)
  quality <- effective_eng + rnorm(N_excl, sd = 0.4)
  ...
  sapply(seq_along(rule_labels), function(k) {
    keep <- switch(k,
      ...
      "8"  = quality > quantile(quality, 0.20),
      "9"  = quality > quantile(quality, 0.10),
...
p_excl <- ggplot(excl_df, aes(x = rule, y = fp_rate, fill = fp_rate > alpha_ex)) +
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.45)) +
  labs(x = "Exclusion rule", y = "False-positive rate (true null effect)",
       title = "Correlated Exclusion Criteria Inflate False Positives — Even Rule by Rule",
       subtitle = paste0(
         "N = ", N_excl, "  \u00b7  True effect = 0  \u00b7  ",
         n_sims_ex, " replications  \u00b7  exclusion criterion correlated with both Y and treatment\n",
         "Blue = near nominal 5%  \u00b7  Red = inflated  \u00b7  Red dashed = rate if researcher picks best rule")) +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(),
...
**What the plot shows:** André (2022) identified two conditions under which an exclusion rule inflates Type I error: the criterion must correlate with (1) the outcome Y and (2) the treatment condition. The SD-based rules (rules 2–4) exclude on Y itself — since Y has zero treatment effect by design, these rules are symmetric across conditions and stay near 5%. The RT and quality rules are where the problem arises. In this study, the treatment is more engaging, which raises participants' response times and quality scores in the treatment condition. Exclusion thresholds are computed pooled (e.g., "bottom 20% of quality across all participants"), so the threshold lands in a region denser with control-group participants. More control participants are removed — and those removed have low engagement and therefore lower Y. The remaining control group now has an artificially elevated mean Y, making it look like the treatment reduced WTP even though the true effect is zero. RT rules show moderate inflation; quality rules (which are more tightly linked to engagement) show the largest inflation. Choosing the best-performing rule across all ten pushes the false-positive rate to the red dashed line — far above the nominal 5%.
