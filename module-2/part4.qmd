...
<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vTcAa2V8uu318T1xdpXblV_-5vE6cA3ei0aNEbsJkLyPRdBV5QZvW8y_hgogfyxqg/pubembed?start=false&loop=false&delayms=3000" width="100%" height="380px" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" title="Module 2 Part 4: Selection Effects"></iframe>
...
sim_pvals <- replicate(n_sims_ex, {
  ...
  # Treatment (eco-label) is more novel and interesting than the control task
  # → it BOOSTS engagement in the treatment condition by 0.5 SD.
  # H₀ is still true on Y: Y depends only on BASELINE engagement, not on the boost.
  engagement_boost <- 0.5
  effective_eng    <- engagement + engagement_boost * trt

  # Y (WTP): driven by baseline engagement, NOT by treatment directly
  Y  <- 0.65 * engagement + sqrt(1 - 0.65^2) * rnorm(N_excl)

  # RT and quality depend on EFFECTIVE engagement (including the boost).
  # Treatment group is more careful → slower RT, higher quality.
  # Applying pooled thresholds therefore excludes more control participants.
  rt      <- 5 + 3 * effective_eng + rnorm(N_excl, sd = 2.5)
  quality <- effective_eng + rnorm(N_excl, sd = 0.6)
...
p_excl <- ggplot(excl_df, aes(x = rule, y = fp_rate, fill = fp_rate > alpha_ex)) +
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.30)) +
  labs(x = "Exclusion rule", y = "False-positive rate (true null effect)",
       ...
       subtitle = paste0(
         ...
         n_sims_ex, " replications  \u00b7  treatment boosts engagement \u2192 asymmetric exclusion\n",
...
**What the plot shows:** The SD-based rules (rules 2–4) exclude based on Y itself — since Y is symmetric across conditions by design, these stay near 5%. The RT and quality rules use *pooled* thresholds (the bottom X% of all participants combined). Because the treatment condition boosts engagement, it shifts the pooled RT and quality distributions rightward — the shared threshold therefore cuts deeper into the *control* distribution, removing more control participants. Those removed have low engagement and thus low Y. The remaining control group has artificially inflated mean Y, making the treatment appear worse by comparison and inflating the false-positive rate for individual rules. Choosing the best-performing rule across all ten pushes it to the red dashed line.
