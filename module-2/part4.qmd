...
The simulation below captures both problems. The DGP includes a latent **engagement** variable that simultaneously drives response time (low engagement → fast RT), outcome quality (low engagement → lower Y), and quality-check performance — but is independent of treatment condition (H₀ is true throughout). Exclusion rules that target engagement-correlated proxies therefore inflate individual-rule FP rates; picking the most favorable rule inflates them further.
...
sim_pvals <- replicate(n_sims_ex, {
  ...
  # Latent engagement: random, NOT related to condition (H0 is true)
  engagement <- rnorm(N_excl)

  # Y is correlated with engagement: engaged participants give more considered
  # responses (e.g., more thoughtful WTP estimates). corr(Y, engagement) = 0.65.
  Y  <- 0.65 * engagement + sqrt(1 - 0.65^2) * rnorm(N_excl)

  # RT: engaged = careful = slower; speeding = fast = low engagement
  rt <- pmax(5 + 3 * engagement + rnorm(N_excl, sd = 2.5), 0.5)

  # Quality score (like an attention/engagement index); correlated with engagement
  quality <- engagement + rnorm(N_excl, sd = 0.6)
...
p_excl <- ggplot(excl_df, aes(x = rule, y = fp_rate, fill = fp_rate > alpha_ex)) +
  ...
  labs(x = "Exclusion rule", y = "False-positive rate (true null effect)",
       ...
       subtitle = paste0(
         ...
         n_sims_ex, " replications  \u00b7  engagement drives both RT/quality and Y\n",
...
**What the plot shows:** The symmetric SD-based rules (rules 2–4) exclude from both tails of Y and stay near the nominal 5%. The RT-based rules exclude from the *low* end of Y (fast responders give lower WTP) and are clearly elevated. The quality-score rules are most aggressive — removing 10–20% of the sample based on a criterion correlated with Y inflates the FP rate substantially even for a single, pre-committed rule. Choosing the best-performing rule across all ten pushes the combined false-positive rate to the red dashed line.
