...
```{r exclusion-bias, fig.height=5.5, fig.width=10}
#| code-summary: "▶ Simulate: false-positive rate across 7 exclusion rules (null effect)"
...
sim_pvals <- replicate(n_sims_ex, {
  ...
  # Baseline engagement: individual difference, independent of treatment assignment.
  engagement <- rnorm(N_excl)

  # Treatment is more engaging. H0 is true: Y has zero treatment effect.
  # Engagement strongly predicts Y and treatment boosts effective engagement --
  # so exclusion criteria are correlated with BOTH the DV (Y) and condition.
  engagement_boost <- 1.0
  effective_eng    <- engagement + engagement_boost * trt

  # Y (WTP): driven by baseline engagement only, not treatment directly.
  ...
  sapply(seq_along(rule_labels), function(k) {
    keep <- switch(k,
      ...
      rt > quantile(rt, 0.05),
      rt > quantile(rt, 0.10),
      quality > quantile(quality, 0.20),
      quality > quantile(quality, 0.10),
      rt > quantile(rt, 0.10) & abs(scale(Y)) < 2.5
    )
    if (sum(keep) < 10) return(1)
    tryCatch(
      t.test(Y[keep & trt == 1], Y[keep & trt == 0])$p.value,
      error = function(e) 1
    )
  })
})  # sim_pvals is 7 x n_sims_ex
...
p_excl <- ggplot(excl_df, aes(x = rule, y = fp_rate, fill = fp_rate > alpha_ex)) +
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1), breaks = seq(0, 0.7, 0.1)) +
