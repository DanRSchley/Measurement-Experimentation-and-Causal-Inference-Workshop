...
::: {.callout-tip}
## Further reading
Scott Cunningham's *Causal Inference: The Mixtape* provides an excellent, accessible treatment of IV with additional examples and intuition. Chapter 7 is freely available at [mixtape.scunning.com/07-instrumental_variables](https://mixtape.scunning.com/07-instrumental_variables).
:::
...
::: {.callout-tip}
## Further reading
For a thorough treatment of RDD with worked examples, see Chapter 6 of Scott Cunningham's *Causal Inference: The Mixtape*, freely available at [mixtape.scunning.com/06-regression_discontinuity](https://mixtape.scunning.com/06-regression_discontinuity).
:::
...
### When the Ruler Is Noisy: Measurement Error in the Running Variable {#sec-rdd-measurement}

The "as-good-as-random near the cutoff" logic is only as strong as the running variable is accurate. This is a direct application of the lesson from Module 1 to the problem of causal identification — and it is routinely overlooked.

Recall the Classical Test Theory decomposition from Module 1, Part 1:

$$X_{\text{obs}} = T_{\text{true}} + \varepsilon$$

Applied to the audit score: the score a product *receives* equals its true environmental quality *plus* auditor noise — different auditors weight criteria differently, visit the factory on different days, and bring different thresholds to bear. The reliability of the audit score is $\rho = \sigma^2_{T} / (\sigma^2_{T} + \sigma^2_{\varepsilon})$. When reliability is high, the observed score closely tracks the truth. When reliability is low, the score is largely noise.

**Why reliability of the running variable is not a minor detail.** The near-threshold quasi-randomness that makes RDD valid depends on units just above and just below the cutoff being genuinely similar in their *true* score. If the audit score is reliable, this is plausible: a product scoring 69 vs. 71 likely has nearly identical true quality, and the two-point gap reflects noise in the measurement instrument. That noise is what creates the local randomness.

But if the audit score is *unreliable* — say, reliability $\rho = 0.60$ — then a product scoring 69 might have a true score of 55 or 80; a product scoring 71 might have a true score anywhere in a wide range. The observed scores of 69 and 71 no longer tell you these products are similar. They are close in the *observed* metric but may be far apart in their *true* quality. Near-threshold units are not quasi-randomly assigned — they are just noisily measured.

::: {.callout-warning}
## Three ways poor measurement undermines RDD

**1. Attenuation of the estimate.** Measurement error in the running variable attenuates the estimated discontinuity toward zero. Because noisy scores misclassify some high-true-score units as below the threshold and vice versa, the sharp step at the cutoff is blurred into a gradual slope. The apparent jump is smaller than the true causal effect — the same attenuation bias that reduces regression coefficients in OLS when predictors are measured with error (Module 1, Part 2).

**2. Corruption of the "as-good-as-random" assumption.** The near-threshold population is only exchangeable when score differences of a few points near the cutoff are attributable to noise, not to genuine differences in true quality. When reliability is low, a two-point gap in observed score could reflect a ten-point gap in true quality. The units on either side of the cutoff are not comparable — the instrument itself has sorted them by its own noise.

**3. Amplification of strategic manipulation.** Units that know the threshold can exploit measurement unreliability to game the score. The Monte Carlo simulation above showed how brands near the cutoff can nudge audit scores across 70. That gaming is *easier* when auditor standards are variable — a second audit or a minor procedural appeal has a higher chance of crossing the threshold when the score has high within-unit variance. Higher noise amplifies the incentive and the feasibility of gaming, making the density test and Type I error problems above even more severe.
:::
...
- **Reliability**: How consistent are scores across repeated assessments of the same unit? If auditors systematically disagree (low inter-rater reliability), the score is noisy, and near-threshold units may not be comparable. Report the inter-rater reliability coefficient.
- **Construct validity**: Does the score measure what it claims to measure, or does it pick up irrelevant variance? An audit that weights easily observable features (energy-efficient lighting) over harder-to-observe ones (supply chain practices) may not reflect the true environmental footprint — and the cutoff then assigns badges based partly on the wrong construct.
- **Convergent validity**: If a second, independent measure of the same underlying construct exists, do they agree? Large disagreement between two audit schemes is evidence of low reliability or construct invalidity in at least one of them.
- **Measurement invariance**: Are audit standards applied consistently across the types of products in your sample? If the same score of 70 means different things for small artisan producers vs. large multinational brands (a form of non-invariance from Module 1, Part 3), the threshold is not a uniform treatment assignment rule.

The practical implication is uncomfortable but important: **the causal precision of an RDD is bounded above by the reliability of its running variable**, in exactly the same way that the power of a mediation analysis is bounded by the reliability of the mediator (Part 4 of this module). A high-quality regression discontinuity study begins with a high-quality measurement instrument for the running variable — not as a convenience, but as a prerequisite for identification.
...
::: {.callout-tip}
## Further reading
Chapter 9 of Scott Cunningham's *Causal Inference: The Mixtape* covers DiD in depth, including staggered adoption and the recent critiques of TWFE. Freely available at [mixtape.scunning.com/09-difference_in_differences](https://mixtape.scunning.com/09-difference_in_differences).
:::
...
**The connection to Module 2, Part 3.** In Module 2, you saw that random assignment does not guarantee exchangeability — it only makes it probabilistically likely, and even then only when the sample is large relative to the number of dimensions of the outcome construct. The playing cards analogy made this vivid: a 52-card deck requires seven riffle shuffles, not one, to approach randomness, because the number of possible orderings grows exponentially with deck size.

DiD is in a harder position than a randomised experiment. At least in a randomised experiment, a *deliberate* randomisation procedure is applied with the specific intent of achieving exchangeability. In DiD, the "treatment assignment" — which country's government happened to pass an eco-label law in 2022 — was determined by political history, lobbying, coalition dynamics, and economic timing. None of these were designed with the goal of creating parallel counterfactual trends. The policy timing is correlated with exactly the forces that drive WTP, because policymakers respond to the same consumer trends and economic conditions that drive WTP.

In the Module 2 framework: the "deck" here has many dimensions (every force that drives WTP over time), and the "shuffle" (policy timing) was not random — it was systematically determined by variables that also affect the outcome. An unrandom shuffle with a large deck is not going to produce anything close to exchangeability.
