...
<iframe src="https://docs.google.com/presentation/d/1tinC2e3yTcwJSDqN2L5nqAZuB4VWV4v0/embed?start=false&loop=false&delayms=3000" width="100%" height="500px" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" title="Module 3 Part 3: Regression Discontinuity Design"></iframe>
...
BW_RDD      <- 10   # fixed local-linear comparison window (±10 audit points)

rdd_mc_one <- function(p_game) {
  mat <- replicate(N_SIM_RDD, {
    # Z: unobserved brand eco-reputation — the confounder.
    # Z is INDEPENDENT of audit score but drives both gaming and WTP.
    Z <- rnorm(N_RDD_MC, 0, 1)

    # Audit scores clustered near the threshold (most brands are borderline).
    # The audit measures operational compliance; brand reputation Z is separate.
    audit <- pmin(100, pmax(30, rnorm(N_RDD_MC, mean = CUTOFF_MC - 3, sd = 9)))

    # High-reputation brands (Z > 0) near threshold appeal/re-audit to cross.
    # cross_gap guarantees the brand actually crosses the threshold;
    # extra scatter places them just above 70 (in [70.5, 77.5]).
    ...
    if (any(game_mask)) {
      ...
      audit[game_mask] <- pmin(100, audit[game_mask] + cross_gap +
                                 runif(sum(game_mask), 0, 7))
    }

    # WTP driven entirely by brand eco-reputation Z — true badge effect = $0.
    # After gaming: just-above 70 is flooded with high-Z brands (high WTP);
    # just-below is depleted of them. Local comparison picks up this
    # Z-sorting as a spurious badge discontinuity.
    WTP_mc <- pmax(1, pmin(10, 5.5 + 1.5 * Z + rnorm(N_RDD_MC, 0, 0.30)))

    # Local linear RDD with a fixed ±BW_RDD window.
    # When WTP has no trend in audit score, rdrobust auto-selects a very wide
    # bandwidth, diluting the gaming signal across hundreds of unaffected
    # observations. A fixed narrow window keeps the comparison local enough
    # that the Z-sorted gamers constitute a substantial fraction of just-above.
    D     <- as.integer(audit >= CUTOFF_MC)
    in_bw <- abs(audit - CUTOFF_MC) <= BW_RDD
    rdd_p <- tryCatch({
      if (sum(in_bw) < 10 || !any(D[in_bw] == 0) || !any(D[in_bw] == 1)) {
        NA_real_
      } else {
        df_bw <- data.frame(y  = WTP_mc[in_bw],
                            xc = audit[in_bw] - CUTOFF_MC,
                            D  = D[in_bw])
        m  <- lm(y ~ D * xc, data = df_bw)
        sm <- summary(m)$coefficients
        2 * pt(-abs(sm["D", "t value"]), df = m$df.residual)
      }
    }, error = function(e) NA_real_)

    # Density test: detects bunching of gamers just above the cutoff
    dens_p <- tryCatch({
...
sim_rdd_mc |>
  ...
  knitr::kable(
    caption = sprintf(
      "RDD simulation: %d sims per condition, N = %d brands, local linear ±10 pts, true badge effect = $0. Gaming selects high-Z brands into just-above 70.",
      N_SIM_RDD, N_RDD_MC)
  )
```
