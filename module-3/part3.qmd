...
```{r iv-first-stage-plot, fig.height=4, fig.width=9}
#| code-summary: "▶ First stage: does Z predict X? And OLS vs. IV coefficient comparison"
p_fs <- df_iv |>
  group_by(Z) |>
...
p_coef <- tibble(
  ...
  ggplot(aes(x = Method, y = Estimate, ymin = lo, ymax = hi, colour = Method)) +
  geom_hline(yintercept = 0.80, linetype = "dashed", colour = "grey40", linewidth = 0.9) +
  annotate("text", x = 0.6, y = 0.82, label = "True effect = $0.80", hjust = 0, size = 3.5) +
  geom_pointrange(size = 1, linewidth = 1.2) +
  scale_colour_manual(values = c("OLS\n(biased)" = clr_ctrl, "IV / 2SLS\n(valid)" = clr_eco),
                      guide = "none") +
  scale_y_continuous(limits = c(0.5, 1.5)) +
  labs(x = NULL, y = "Estimated eco-label effect ($)",
       title = "OLS overstates the effect; IV recovers the truth",
       subtitle = "OLS bias: U drives both adoption and WTP, inflating the naive comparison") +
  theme_mod3()
