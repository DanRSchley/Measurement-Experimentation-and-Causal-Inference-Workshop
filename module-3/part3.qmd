...
**The mechanism.** Two variables matter here: (1) the *audit score* — what the running variable measures — and (2) latent *brand eco-credential strength* Z, an unobserved dimension reflecting the brand's genuine reputation, loyal customer base, and eco-marketing sophistication. Z is largely independent of the audit score (a firm can have strong green credentials yet score just below 70 due to measurement noise or procedural quirks). But Z does two things: it motivates gaming (high-Z firms near the threshold believe they deserve the badge and have the resources to appeal or re-submit) and it independently raises customer WTP (eco-conscious shoppers follow high-credential brands). **The true badge effect is zero.** After gaming, just above 70 contains genuine scorers (typical Z) *plus* gaming high-credential brands (high Z); just below, high-Z firms have departed, leaving a depleted, lower-Z pool. The RDD compares these two compositionally different groups and detects a WTP gap driven entirely by Z-sorting, not the badge.
...
N_RDD_MC    <- 500
CUTOFF_MC   <- 70
GAME_WINDOW <- 8    # firms within 8 pts below cutoff are at risk of gaming

rdd_mc_one <- function(p_game) {
  mat <- replicate(N_SIM_RDD, {
    # Z: latent brand eco-credential strength — the unobserved confounder
    # Z is independent of the audit score but drives both gaming and WTP
    Z <- rnorm(N_RDD_MC, 0, 1)

    # Audit score reflects firm-level environmental performance,
    # largely independent of brand reputation Z
    qual_raw <- runif(N_RDD_MC, 30, 100)
    audit    <- pmin(100, pmax(30, qual_raw + rnorm(N_RDD_MC, 0, 4)))

    # High-credential firms (Z > 0) just below threshold game the audit score.
    # After gaming they land just above 70 — with their high-Z WTP intact.
    near_below <- audit >= (CUTOFF_MC - GAME_WINDOW) & audit < CUTOFF_MC
    game_mask  <- near_below & (Z > 0) & (runif(N_RDD_MC) < p_game)
    audit[game_mask] <- pmin(100, audit[game_mask] +
                               runif(sum(game_mask), 1, GAME_WINDOW + 2))

    # WTP driven entirely by brand credentials Z — badge has NO causal effect.
    # Just-above post-gaming: surplus of high-Z firms. Just-below: deficit.
    # RDD picks up the Z-induced WTP gap as a spurious badge effect.
    WTP_mc <- 4.0 + 1.0 * Z + rnorm(N_RDD_MC, 0, 0.50)
...
::: {.callout-warning}
## The key pattern: Type I error climbs before the density test has power to catch it

At low gaming rates (10–20%), the density test — the standard diagnostic — has low power to detect manipulation. Yet the RDD Type I error is already substantially above 5%. A researcher who runs `rddensity`, sees p > .05, and proceeds in good faith will still produce spurious findings at these rates.

**What to do:** (1) Report the density test but be explicit about its power at your sample size. (2) Run covariate continuity checks — if gaming firms have systematically different pre-existing characteristics, those will show up as covariate discontinuities. (3) Narrow the bandwidth: at very narrow bandwidths, gaming firms are still present above but fewer are present below (since the window is smaller), reducing the sorting imbalance. (4) If qualitative knowledge suggests manipulation is plausible, treat the RDD as a *fuzzy* design and instrument.
:::
