...
rdd_mc_one <- function(p_game) {
  # replicate() returns a 2×N matrix; no strict-length requirement like vapply
  mat <- replicate(N_SIM_RDD, {
    ...
    rdd_p <- tryCatch({
      ...
      2 * pnorm(-abs(est$coef[1] / est$se[1]))  # [1] = Conventional; safe indexing
    }, error = function(e) NA_real_)

    dens_p <- tryCatch({
      rd <- rddensity(X = audit, c = CUTOFF_MC)
      p  <- rd$test$p_jk          # works in rddensity >= 2.1
      if (is.null(p) || !is.finite(p)) 1.0 else as.numeric(p)[1]
    }, error = function(e) 1.0)   # default 1.0: "test sees nothing" when it errors
