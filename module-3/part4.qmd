...
### Violation Severity at Fixed 2 Pre-Periods {#sec-pt-violation-severity}

The previous simulation varied how many pre-periods you collect, holding the violation at \$0.15/yr. Now we fix pre-periods at 2 — the single most common situation in management and marketing research — and ask: **how large does the differential trend need to be before DiD becomes unreliable?**

The answer is uncomfortable: even a \$0.05/yr differential ($\approx 1\%$ of baseline WTP) produces Type I error well above 5%. A \$0.15/yr trend — modest and plausible in many real markets — produces Type I error around 40%.

```{r pt-violation-severity, fig.height=5, fig.width=10}
#| code-summary: "▶ Monte Carlo: Type I error vs. differential trend magnitude (2 pre-periods fixed)"
set.seed(2026)
N_SIM_SEV   <- 300
N_PRE_FIXED <- 2     # fixed at 2 pre-periods
slope_grid  <- seq(0, 0.40, by = 0.05)   # differential trend: $0/yr to $0.40/yr

pt_sev_one <- function(diff_slope) {
  years_s <- c(seq(T_treat_v - N_PRE_FIXED, T_treat_v - 1),
               T_treat_v:(T_treat_v + n_post_v - 1))
  n_yrs   <- length(years_s)

  pvec <- replicate(N_SIM_SEV, {
    n_obs     <- n_stores_v * n_yrs
    store_s   <- rep(1:n_stores_v,  each  = n_yrs)
    year_s    <- rep(years_s,       times = n_stores_v)
    treated_s <- as.numeric(store_s <= n_dutch_v)
    post_s    <- as.numeric(year_s >= T_treat_v)
    did_s     <- treated_s * post_s
    diff_s    <- treated_s * diff_slope * (year_s - T_treat_v)
    fe_s      <- rnorm(n_stores_v, 0, 0.6)[store_s]
    WTP_s     <- 5.20 + fe_s + 0.08 * (year_s - T_treat_v) + diff_s + rnorm(n_obs, 0, 0.40)

    df_s <- data.frame(WTP   = WTP_s, did   = did_s,
                       store = factor(store_s), year  = factor(year_s))
    fit  <- lm(WTP ~ did + store + year, data = df_s)
    tryCatch(
      coeftest(fit, vcovCL(fit, cluster = ~store))["did", 4],
      error = function(e) NA_real_
    )
  })

  data.frame(diff_slope = diff_slope, pval = pvec)
}

sim_sev <- map_dfr(slope_grid, pt_sev_one)
cat(sprintf("Severity simulation complete: %d conditions × %d sims = %d total.\n",
            length(slope_grid), N_SIM_SEV, nrow(sim_sev)))
```

```{r pt-severity-plot, fig.height=5, fig.width=9, echo=FALSE}
sev_summary <- sim_sev |>
  group_by(diff_slope) |>
  summarise(t1_rate = mean(pval < 0.05, na.rm = TRUE), .groups = "drop")

ggplot(sev_summary, aes(x = diff_slope, y = t1_rate)) +
  geom_hline(yintercept = 0.05, linetype = "dotted", colour = "grey40", linewidth = 0.9) +
  annotate("text", x = 0.01, y = 0.075, label = "Nominal 5%", size = 3.2,
           colour = "grey40", hjust = 0) +
  geom_vline(xintercept = diff_slope_v, linetype = "dashed",
             colour = "#e67e22", linewidth = 0.8) +
  annotate("text", x = diff_slope_v + 0.005, y = 0.85,
           label = sprintf("$%.2f/yr used\nin main example", diff_slope_v),
           hjust = 0, size = 3.0, colour = "#e67e22") +
  geom_line(colour = "firebrick", linewidth = 1.3) +
  geom_point(colour = "firebrick", size = 3) +
  geom_ribbon(aes(
    ymin = qbeta(0.10, t1_rate * N_SIM_SEV + 0.5, (1 - t1_rate) * N_SIM_SEV + 0.5),
    ymax = qbeta(0.90, t1_rate * N_SIM_SEV + 0.5, (1 - t1_rate) * N_SIM_SEV + 0.5)),
    alpha = 0.15, fill = "firebrick") +
  scale_x_continuous(labels = function(x) sprintf("$%.2f/yr", x)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    x        = "Differential trend between Dutch and Belgian stores (true effect = $0)",
    y        = "Type I error rate",
    title    = "DiD Type I error rises sharply with violation magnitude (2 pre-periods, true effect = $0)",
    subtitle = sprintf(
      "Shaded band = 80%% simulation interval  |  %d sims per condition  |  %d Dutch, %d Belgian stores",
      N_SIM_SEV, n_dutch_v, n_belgian_v)
  ) +
  theme_mod3()
```

```{r pt-severity-table, echo=FALSE}
sev_summary |>
  transmute(
    `Differential trend ($/yr)` = sprintf("$%.2f", diff_slope),
    `Type I error rate`         = sprintf("%.0f%%", 100 * t1_rate),
    `Relative to nominal 5%`    = sprintf("%.1f×", t1_rate / 0.05)
  ) |>
  knitr::kable(
    caption = sprintf(
      "DiD Type I error by violation severity: 2 pre-periods, %d sims per condition, true effect = $0.",
      N_SIM_SEV)
  )
```

::: {.callout-important}
## What "plausible" violations look like in practice

A \$0.15/yr differential trend across Dutch and Belgian stores is not an exotic failure — it corresponds to one group's customer base shifting toward greener preferences at a modest 3% annual rate. Yet this produces a Type I error above 30% with 2 pre-periods. **The pre-trend test with 2 pre-periods has essentially no power to catch it** (as shown in the previous simulation), so a researcher following standard practice would see clean pre-trends and proceed with confidence.

This is why theoretical justification for the control group matters at least as much as the statistical pre-trend test. Ask not just "do the lines look parallel before treatment?" but "is there any reason to believe Dutch and Belgian shoppers were evolving differently for reasons unrelated to the law?"
:::
