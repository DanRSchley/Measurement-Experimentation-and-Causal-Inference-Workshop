...
The "acyclic" part means no variable can cause itself through any chain of arrows — time must flow forward. You can draw feedback loops if you think about each time-point as a separate node (e.g., $X_t \rightarrow Y_t \rightarrow X_{t+1}$), but within a cross-sectional DAG, cycles are forbidden.
...
**Chain (mediator):** $X \rightarrow M \rightarrow Y$. $X$ causes $M$, $M$ causes $Y$. $X$ and $Y$ are associated, but the association runs *through* $M$. If you block $M$ — by controlling for it in a regression — you sever the path and the $X$–$Y$ association disappears.

**Fork (confounder):** $X \leftarrow U \rightarrow Y$. A common cause $U$ produces an association between $X$ and $Y$ even if $X$ does not cause $Y$ at all. Controlling for $U$ blocks this spurious path.

**Collider:** $X \rightarrow M \leftarrow Y$. Both $X$ and $Y$ cause $M$, but $X$ and $Y$ are *not* associated in the population — the arrows both flow *into* $M$, so the path is blocked. If you condition on $M$ (control for it, or stratify on it, or select a sample where $M$ is constant), you *open* a spurious association between $X$ and $Y$ that did not previously exist. This is collider bias, and it is one of the most counterintuitive results in causal inference.
...
Suppose you study whether conscientiousness ($X$) causes job performance ($Y$). You decide to study only employees who were *hired* ($M = 1$). Hiring is itself caused by both conscientiousness (conscientious applicants get hired) and prior performance signals ($Y$-precursors). By restricting to hired employees, you have conditioned on $M$ — a collider — and opened a spurious *negative* association between conscientiousness and performance *within your sample*. Conscientious employees who were hired despite mediocre performance signals are uncommon; within the hired group, high conscientiousness and high performance are no longer independent. This is why studies of elite samples (top MBA programmes, Fortune 500 firms, clinical trial completers) routinely find attenuated or even reversed relationships compared to population samples.
...
These are equal only when $X$ is not confounded. If conscientiousness affects hiring and hiring is measured as a collider, observing $X = x$ and *setting* $X = x$ produce different distributions of $Y$ — even within the same data.

Random assignment is the cleanest implementation of $\mathit{do}(X = x)$: by forcing $X$ to the value you choose, you sever all incoming arrows to $X$ (you replace the natural causes of $X$ with your randomiser). The resulting $P(Y \mid \mathit{do}(X))$ is identified without any further adjustment. When you cannot randomise, identification requires finding an *adjustment set* that mimics what randomisation would have done.

### The backdoor criterion {#sec-backdoor}

A **backdoor path** from $X$ to $Y$ is any path that starts by going *backwards* along an incoming arrow into $X$. Backdoor paths carry confounding association — they are association between $X$ and $Y$ that does not flow through any causal effect of $X$ on $Y$.

The **backdoor criterion** says: a set of variables $Z$ is sufficient to identify the causal effect of $X$ on $Y$ if $Z$ (1) blocks all backdoor paths from $X$ to $Y$, and (2) does not include any descendant of $X$ (no post-treatment variables or mediators).
...
The mediator structure ($X \rightarrow M \rightarrow Y$) creates a choice: do you control for $M$ or not?

- **Do not control for $M$** if you want the **total effect** of $X$ on $Y$ — the sum of the direct path ($X \rightarrow Y$) and the indirect path ($X \rightarrow M \rightarrow Y$). Controlling for $M$ blocks the indirect path and gives you only the direct effect.

- **Control for $M$** only if you want the **direct effect** of $X$ on $Y$ holding $M$ fixed — and *only* if $M$ is not itself confounded with $Y$ (i.e., there is no unmeasured common cause of $M$ and $Y$). If there is, controlling for $M$ opens a new backdoor path and introduces bias rather than removing it.

In the eco-label context: suppose the label works by (a) directly signalling quality and (b) activating guilt about environmental harm ($M$), which then raises WTP. If you want to know "does the label raise WTP?", do not control for guilt — you would remove the mechanism. If you want to know "does the label raise WTP *beyond* the guilt pathway?", control for guilt — but only after checking that guilt is not confounded with WTP by some unmeasured variable.
...
The classic example: smoking ($X$) causes cancer ($Y$) through tar deposits in the lungs ($M$). $U$ = genetic predisposition confounds the $X$–$Y$ relationship. But if we can measure $M$, and $M$ is not directly confounded with $Y$ except through pathways that $X$ blocks, we can first estimate $P(M \mid \mathit{do}(X))$ (randomise $X$ given $M$ blocks $U$) and then $P(Y \mid \mathit{do}(M))$ (using $X$ to block the backdoor into $M$), and combine them.

In management research, the frontdoor is relevant when you have a clear causal chain — policy $\rightarrow$ behaviour $\rightarrow$ outcome — and you can measure the behaviour even though the policy's take-up is confounded. It is less commonly exploited than instrumental variables but rests on weaker assumptions when the structure is present.
