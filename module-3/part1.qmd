...
//
//   regDX — regression Y~X+M: X IS significant  → my≠0; xy≠0
//            (overridden if medD0 also active — mediation supersedes regression)
//   regD0 — regression Y~X+M: X NOT significant → my≠0; xy=0
//            (overridden if medDX also active — mediation supersedes regression)
//   expX  — X experimentally manipulated         → X exogenous: xm≠2,3; xy≠2,3
//   medDX — mediation, partial (direct ≠ 0)      → xm≠0; my≠0; xy≠0 (always enforced)
//   medD0 — mediation, pure   (direct = 0)       → xm≠0; my≠0; xy=0 (always enforced)
//
// Mediation supersedes regression on the xy constraint: if medD0 is active,
// regDX's xy≠0 requirement is waived; if medDX is active, regD0's xy=0 is waived.
function dagConsistent(s, active) {
  ...
  for (const t of active) {
    if (t === "regDX") {                                      // X→Y direct effect found
      if (!any(my)) return false;
      if (!active.includes("medD0") && !any(xy)) return false; // medD0 supersedes
    }
    if (t === "regD0") {                                      // no X→Y direct effect
      if (!any(my)) return false;
      if (!active.includes("medDX") && xy !== 0) return false; // medDX supersedes
    }
    if (t === "medDX" && !(any(xm) && any(my) && any(xy))) return false; // partial mediation
    if (t === "medD0" && !(any(xm) && any(my) && xy === 0)) return false; // pure mediation
  }
  return true;
}
```
...
viewof activeTools = {
  const TOOLS = [
    { id:"regDX", cat:"Regression", label:"Regression: X→Y effect",       sub:"X has a direct effect on Y (xy≠0); M also predicts Y" },
    { id:"regD0", cat:"Regression", label:"Regression: no X→Y effect",    sub:"X has no direct effect on Y (xy=0); M predicts Y" },
    { id:"expX",  cat:"Experiment", label:"Manipulate X",                  sub:"X is exogenous — removes all arrows into X" },
    { id:"medDX", cat:"Mediation",  label:"Mediation: partial (direct≠0)", sub:"X→M→Y indirect path plus direct X→Y effect" },
    { id:"medD0", cat:"Mediation",  label:"Mediation: pure (direct=0)",    sub:"X→M→Y indirect path only; no direct X→Y" },
  ];
...
//
//   regDX2 — regression: X IS significant    → my≠0; xy≠0 (overridden by medD02)
//   regD02 — regression: X NOT significant   → my≠0; xy=0 (overridden by medDX2)
//   expX2  — manipulate X                    → X exogenous: xm≠2,3; xy≠2,3; ux≠2,3
//   medDX2 — mediation, partial (direct≠0)   → xm≠0; my≠0; xy≠0 (always enforced)
//   medD02 — mediation, pure   (direct=0)    → xm≠0; my≠0; xy=0 (always enforced)
// Mediation supersedes regression on the xy constraint (same rule as sim 1).
dagConsistent2 = (s, active) => {
  ...
  for (const t of active) {
    if (t === "regDX2") {
      if (!any(my)) return false;
      if (!active.includes("medD02") && !any(xy)) return false;
    }
    if (t === "regD02") {
      if (!any(my)) return false;
      if (!active.includes("medDX2") && xy !== 0) return false;
    }
    if (t === "medDX2" && !(any(xm) && any(my) && any(xy))) return false;
    if (t === "medD02" && !(any(xm) && any(my) && xy === 0)) return false;
  }
  return true;
}
```
...
viewof activeTools2 = {
  const TOOLS = [
    { id:"regDX2", cat:"Regression", label:"Regression: X→Y effect",       sub:"X has a direct effect on Y (xy≠0); M also predicts Y" },
    { id:"regD02", cat:"Regression", label:"Regression: no X→Y effect",    sub:"X has no direct effect on Y (xy=0); M predicts Y" },
    { id:"expX2",  cat:"Experiment", label:"Manipulate X",                  sub:"X is exogenous — removes arrows into X, including U→X" },
    { id:"medDX2", cat:"Mediation",  label:"Mediation: partial (direct≠0)", sub:"X→M→Y indirect path plus direct X→Y effect" },
    { id:"medD02", cat:"Mediation",  label:"Mediation: pure (direct=0)",    sub:"X→M→Y indirect path only; no direct X→Y" },
  ];
  const ICONS = { Regression:"📊", Experiment:"🧪", Mediation:"⛓️" };
  const active = new Set();
  const buttons = [];
...
1. **You have measured the right moderators.** You can only condition on variables you have collected. If the strongest driver of heterogeneity is something unmeasured — say, a consumer's specific shopping occasion or the brand's existing equity — your CATEs are averages over that unobserved heterogeneity and may not correspond to any real subgroup's true effect.
...
- **Exclusion restriction:** Assignment $Z$ affects the outcome *only through* take-up $D$ — not through any direct path. If displaying eco-labels changes store atmosphere and affects WTP independently of whether any consumer reads them, the exclusion restriction fails.
