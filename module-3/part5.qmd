---
title: "Part 5: Mediation Analysis"
---
...
<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vSYMsPUAv6Ag5bGkOFdm8ivmMbS_Tdg4Lz-lVOh8GM2TzZAnXLgUcUJCMl_SBUYoQ/pub?start=false&loop=false&delayms=3000" width="100%" height="500px" frameborder="0" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true" title="Module 3 Part 5: Mediation Analysis"></iframe>
...
## Type I Error from Discriminant Validity Failure {#sec-med-discval}

Scenarios 1–3 above assumed that perceived sustainability (M) and WTP (Y) are measuring distinct constructs. But what if they partially reflect the same underlying disposition — "eco-identity" — that a researcher has failed to separate? This is the discriminant validity problem from Module 1 applied directly to mediation.

**The mechanism.** Suppose there is a latent factor η ("eco-identity") that independently causes both high perceived sustainability ratings and high WTP. The researcher does not measure η — they only see M and Y. When η is active, M and Y are correlated *even when there is no causal M → Y path*. The mediation model mistakes this construct-induced correlation for evidence of a b-path. **The true ACME = 0**, but OLS reports a large, significant indirect effect.
...
**A moderate r(M, Y) of 0.40 already produces Type I error around 40–50%.** Most researchers would not flag r = 0.40 as a discriminant validity crisis — HTMT would be around 0.40 as well, well below the 0.85 warning threshold. Yet the indirect effect estimate is almost certainly spurious if M and Y share a latent common cause.

**The connection to Module 1 is direct.** If you would not accept two scales as measuring *different* constructs in a CFA (Module 1, Case 1), you cannot treat their relationship as evidence of a mediation *mechanism*. The b-path in mediation is the OLS slope from M on Y — if M and Y are both imperfect measures of the same thing (eco-identity, health consciousness, general positivity), that slope estimates the reliability of the shared dimension, not a causal effect.

**Practical implication:** Before running mediation, test HTMT(M, Y). Values above 0.70 warrant explicit theoretical justification for why M and Y measure distinct constructs. Values above 0.85 indicate that any apparent ACME is more parsimoniously explained by discriminant validity failure than by a real mechanism. The Imai sensitivity parameter ρ* will be small in exactly this scenario — run `medsens()` and treat a ρ* below 0.30 as a red flag that the result is artefactual.
:::
...
## Researcher Checklist: Causal Mediation Analysis {#sec-checklist-part5-m3}
...
- [ ] **Is the mediator measured reliably and discriminantly valid from the outcome?** Measurement error in M attenuates the a-path; HTMT(M, Y) near 1.0 means mediation is picking up measurement contamination, not a causal pathway.
