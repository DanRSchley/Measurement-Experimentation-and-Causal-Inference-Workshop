...
pkgs <- c(
  ...
  "conflicted",
  "MatchIt", "WeightIt", "cobalt", "optmatch",
  "rdrobust", "rddensity",
  "did",
  "mediation", "robmed",
...
### Decision 3: Nearest-neighbour greedy vs. optimal vs. full matching
...
**Full matching** assigns *every* observation — treated and control — to a matched subclass, with a variable number of controls per treated unit. No data is ever discarded. This is the most statistically efficient approach because all observations contribute to the estimate, and it targets the ATE (not just the ATT) when treated and control units are both matched. The outcome analysis uses subclass weights rather than a simple mean difference. Full matching is particularly useful when the treated group is a small fraction of the sample.
...
The chart below demonstrates that of all the decisions above, getting the PS model specification right dominates all others. We compare a misspecified logit (main effects only) against a model that correctly captures the `env × income` interaction driving treatment in our DGP.

```{r psm-model-spec, fig.height=4.5, fig.width=10}
#| code-summary: "▶ Compare misspecified vs. correctly specified PS models"
# ── YOUR DATA: replace treat ~ ... with your treatment indicator and
#    pre-treatment covariates. Try multiple PS formulas and check whether
#    estimates shift — sensitivity across specifications is a red flag.
# ── KEY ARGS: distance="logit" / "probit" selects the PS model family;
#    caliper (in SD units when std.caliper=TRUE) discards pairs whose PS
#    difference exceeds the threshold, preventing poor matches.

# ── Misspecified: logit with only main effects (misses env:income interaction)
m_ps_mis1 <- matchit(treat ~ env + price_sens + income,
                     data=df_obs, method="nearest", distance="logit")

# ── Misspecified: probit + poly(env,2) — adds flexibility but still misses
#    the env:income interaction that actually drives treatment assignment
m_ps_mis2 <- matchit(treat ~ poly(env,2) + price_sens + income,
                     data=df_obs, method="nearest", distance="probit",
                     caliper=0.15, std.caliper=TRUE)

# ── Correctly specified: logit with env² and env×income matching the true DGP
m_ps_cor  <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                     data=df_obs, method="nearest", distance="logit",
                     caliper=0.15, std.caliper=TRUE)

results_psm_spec <- map_dfr(
  list("1. Logit — main effects only\n(misspecified)"          = m_ps_mis1,
       "2. Probit + poly(env,2) + caliper\n(misspecified)"     = m_ps_mis2,
       "3. Logit + env² + env×income + caliper\n(correct PS)"  = m_ps_cor),
  function(m) {
    d   <- match.data(m)
...
results_psm_spec |>
  ...
  labs(x="ATT estimate — eco-label WTP premium ($)", y=NULL,
       ...
       title="Misspecifying the PS model produces substantial bias",
       subtitle="The misspecified logit misses the env×income interaction — both misspecified models overshoot the true ATT") +
  theme_mod3()
```

Notice that the misspecified models do not merely add noise around the true ATT — they systematically *overshoot* it. The reason is structural: the `env × income` interaction means the most environmentally conscious *and* wealthiest consumers have especially high treatment probabilities *and* especially high baseline WTP. A PS model that misses this interaction assigns these consumers too low a probability of treatment, so they are matched to controls who are actually quite different in unmeasured ways. The matched sample is contaminated by selection on the very dimension the model failed to capture.

::: {.callout-warning}
## Matching can look balanced and still be biased

A matched sample from the misspecified logit will show excellent marginal balance — similar means on `env`, `income`, and `price_sens` across groups. But `env × income` (a covariate not explicitly in the balance check) will still be badly imbalanced. **Always check interaction terms and polynomial terms in your Love plot**, not just the main effects you put in the PS model.
:::

### Matching design trade-offs: 1:1 vs. 1:k vs. full matching
...
#| code-summary: "▶ Compare 1:1, 1:3, 1:5, with-replacement, and full matching (correct PS)"
# ── All approaches use the correctly-specified PS (env² + env×income).
...
m_11  <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                 data=df_obs, method="nearest", distance="logit",
...
m_13  <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                 data=df_obs, method="nearest", distance="logit",
                 ratio=3, replace=FALSE, caliper=0.15, std.caliper=TRUE)

# 1:5 nearest-neighbour without replacement
m_15  <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                 data=df_obs, method="nearest", distance="logit",
...
m_wr  <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                 data=df_obs, method="nearest", distance="logit",
                 ratio=1, replace=TRUE, caliper=0.15, std.caliper=TRUE)

# Full matching — every observation assigned to a subclass; no data discarded
# Targets ATT among treated units using subclass weights
m_ful <- matchit(treat ~ env + price_sens + income + env_sq + env_income,
                 data=df_obs, method="full", distance="logit")
...
results_psm_approach |>
  ...
  labs(x="ATT estimate — eco-label WTP premium ($)", y=NULL,
       ...
       subtitle="With-replacement and full matching use more data; 1:1 without replacement keeps the cleanest pairs") +
  theme_mod3()
```

The key takeaway: **once the PS is correctly specified, the matching design choices affect precision more than they affect bias**. With-replacement matching and full matching typically use more of the data and therefore yield narrower confidence intervals, but they require more care in outcome analysis (correct SE formulas for with-replacement; subclass weights for full matching). For exploratory work, 1:1 without replacement plus a caliper is the cleanest starting point.
