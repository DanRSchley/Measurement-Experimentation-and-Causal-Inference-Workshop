...
# Part 1: What Do We Mean by "Causal Effect"?
...
# Part 3: Regression Discontinuity Design
...
# Part 5: Mediation — Does the Eco-Label Work *Through* Perceived Sustainability?
...
out_sgn <- sample(c(-1, 1), out_n, replace = TRUE)
...
ggplot(sens_df, aes(x = rho, y = ACME, colour = Scenario)) +
  ...
  { if (!is.na(crit_rho_s1))
      list(
        ...
        geom_vline(xintercept = -crit_rho_s1, linetype = "dashed",
                   colour = clr_eco, linewidth = 0.9),
        annotate("text", x = crit_rho_s1 + 0.03, y = 0.06,
                 label = sprintf("\u03c1*= %s\n(clean \u2014 ROBUST)", fmt_rho(crit_rho_s1)),
                 hjust = 0, size = 3.3, colour = clr_eco)
      ) } +
  { if (!is.na(crit_rho_s3))
      list(
        ...
        geom_vline(xintercept = -crit_rho_s3, linetype = "dashed",
                   colour = clr_ctrl, linewidth = 0.9),
        annotate("text", x = crit_rho_s3 + 0.03, y = -0.05,
                 label = sprintf("\u03c1*= %s\n(confounded \u2014 FRAGILE)", fmt_rho(crit_rho_s3)),
                 hjust = 0, size = 3.3, colour = clr_ctrl)
      ) } +
  scale_colour_manual(
...
```{r mediation-bias-comparison, fig.height=7, fig.width=11, echo=FALSE}
...
## The Potential Outcomes Framework {#sec-mediation-imai}

Imai, Keele & Yamamoto (2010) reframe mediation in **potential outcomes** terms:

> If we forced M to the value it would take under X = 0, but kept X = 1, what would Y be?

The **ACME** = how much of X's effect on Y operates *through* changing M. The critical assumption — **sequential ignorability** — says no unmeasured confounders of M→Y given X. Scenario 3 violates this. The sensitivity analysis makes the violation's consequences explicit and quantifiable.

```{r mediation-imai-detail, fig.height=4, fig.width=8}
#| code-summary: "▶ Imai mediate() on clean DGP with bootstrap CIs"
m_m_s1  <- lm(perc_sust ~ eco_label, data = df_s1)
m_y_s1  <- lm(WTP ~ eco_label + perc_sust, data = df_s1)
set.seed(99)
med_imai_full <- mediate(m_m_s1, m_y_s1, treat = "eco_label",
                         mediator = "perc_sust", boot = TRUE, sims = 1000)
summary(med_imai_full)

# Replace base-R plot() (which emits 4 separate figures) with a single ggplot
tibble(
  Quantity = factor(
    c("ACME\n(indirect)", "ADE\n(direct)", "Total\neffect", "Prop.\nmediated"),
    levels = c("Prop.\nmediated", "Total\neffect", "ADE\n(direct)", "ACME\n(indirect)")
  ),
  Estimate = c(med_imai_full$d0,         med_imai_full$z0,
               med_imai_full$tau.coef,   med_imai_full$n0),
  CI_lo    = c(med_imai_full$d0.ci[1],   med_imai_full$z0.ci[1],
               med_imai_full$tau.ci[1],  med_imai_full$n0.ci[1]),
  CI_hi    = c(med_imai_full$d0.ci[2],   med_imai_full$z0.ci[2],
               med_imai_full$tau.ci[2],  med_imai_full$n0.ci[2])
) |>
  ggplot(aes(x = Estimate, xmin = CI_lo, xmax = CI_hi, y = Quantity)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey50", linewidth = 0.9) +
  geom_pointrange(colour = clr_eco, size = 0.7, linewidth = 1.1) +
  geom_text(aes(label = sprintf("%.3f\n[%.3f, %.3f]", Estimate, CI_lo, CI_hi)),
            hjust = -0.12, size = 3, colour = "grey20") +
  scale_x_continuous(expand = expansion(mult = c(0.05, 0.35))) +
  labs(x = "Estimate (bootstrap 95% CI, 1 000 sims)", y = NULL,
       title = "Imai mediate(): all four quantities on clean DGP (Scenario 1)",
       subtitle = "ACME = indirect effect (a \u00d7 b)  |  ADE = direct effect (c\u2019)  |  Total = ADE + ACME") +
  theme_mod3()
```

### Parameter Recovery on Clean DGP {#sec-mediation-comparison-table}

```{r mediation-truth-comparison, echo=FALSE}
#| code-summary: "▶ All three estimators vs. true values (clean DGP)"
tibble(
  Parameter       = c("a path  (X → M)", "b path  (M → Y)", "ACME  (indirect)",
                      "ADE  (direct)", "Total effect"),
  `True value`    = c(a_true, b_true, ACME_true, c_prime, c_prime + ACME_true),
  `OLS`           = round(c(res_s1$a_hat, res_s1$b_hat,
                             res_s1$ols_est,
                             med_imai_full$z0,
                             med_imai_full$tau.coef), 3),
  `ROBMED`        = round(c(NA, NA, res_s1$rob_est, NA, NA), 3),
  `95% CI (OLS/Imai)` = c("—", "—",
    sprintf("[%.3f, %.3f]", res_s1$ols_lo, res_s1$ols_hi),
    sprintf("[%.3f, %.3f]", med_imai_full$z0.ci[1],  med_imai_full$z0.ci[2]),
    sprintf("[%.3f, %.3f]", med_imai_full$tau.ci[1], med_imai_full$tau.ci[2]))
) |>
  knitr::kable(caption = "Parameter recovery on clean DGP (N = 350, normal errors)",
               digits = 3, na.rep = "—")
```
...
- **Module 1** (measurement): if perceived sustainability and WTP both reflect the same latent *green attitude*, their shared variance mimics Scenario 3's U-confound. Testing HTMT and discriminant validity guards against this before mediation is attempted.
- **Module 2** (hypothesis testing): the RCT establishes X→Y with high confidence. Mediation adds the claim X→M→Y, which rests on sequential ignorability — an assumption the RCT does not guarantee.
- **ROBMED** handles realistic data quality problems (Scenario 2); Imai's sensitivity analysis quantifies identification fragility (Scenario 3). Use both, and be honest about what each can and cannot rule out.
:::
